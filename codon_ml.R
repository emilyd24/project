...
#Reading in the fasta file and lines 
#fasta_file = "seqfile.fasta"
...
for (seqfile in seqfiles) {
  ...
  #Loop through the lines of the FASTA file, extracting species names - e.g. >emily_dunican will now be species, emily_dunican. Identifying the species name by looking at the > in front of the name 
  ...
  merged_data$adult_weight.g. = NULL
  
  merged_data = merged_data[complete.cases(merged_data), ]
  
  #Print the merged dataset which will be used in the tree 
  #print(merged_data)
  
  
  
  
  #editing the original tree by removing all of the non common species - needed to make a vector called not_in_merged_species and remove that from the tree
  not_in_merged_species = setdiff(continuous_species, merged_data$Species)
  #print(not_in_merged_species)
  #length(not_in_merged_species)
  
  
  
  
  
  # Filter the tree to the subset of species - using the not_in_merged_species vector to drop those species from the tree, result is the subset_tree, a subset of the orginal tree that contains the overlapping species we have 
  #subset_tree <- drop.tip(original_tree, setdiff(original_tree$tip.label, subset_species))
  ...
  #attach data to tree, make matrix 
  lifespan_data = as.matrix(merged_data$max_longevity.months.)
  #make numeric
  lifespan_data = as.numeric(lifespan_data)
  #add Species
  #add labels back
  lifespan_data = setNames(lifespan_data,merged_data$Species)
  
  
  # Reconstruct ancestral states
  anc_states = fastAnc(subset_tree, lifespan_data, vars=FALSE, CI=FALSE)
  anc_states_df = data.frame(node = as.numeric(names(anc_states)), reconstructed = as.numeric(anc_states))
  contMap(subset_tree,lifespan_data)
  
  # reconstruct ancestral states (maximum likelihood model)
  anc <- fastAnc(subset_tree, lifespan_data)
  anc_df <- data.frame(node = as.numeric(names(anc)), reconstructed = as.numeric(anc))
  # create dataframe of branches
  ...
  tip_label_to_node <- left_join(tip_label_to_node, merged_data[,c("Species","max_longevity.months.")])
  #fill in terminal node values
  branches <- left_join(branches, tip_label_to_node, by = c("node2"="node")) %>% dplyr::select(-Species)
  branches <- branches %>%
    mutate(reconstructed_node2 = ifelse(is.na(reconstructed_node2), max_longevity.months., reconstructed_node2)) %>%
    dplyr::select(-max_longevity.months.)
  ...
  #file_path_newick <- "/Users/emilydunican/Desktop/Project/mytree.newick"
  #file_path_nexus <- "/Users/emilydunican/Desktop/Project/mytree.nexus"
  
  # Save the tree in Newick format
  #write.tree(subset_tree, file = file_path_newick)
  
  # Save the tree in Nexus format
  #write.nexus(subset_tree, file = file_path_nexus)
  
  
  treefile = subset_tree  
  
  #Creating a .csv file that contains the average values of the lifespan, which will be read in 
  ...
  ## calculate the vector lifespan, each entry of which is the average lifespan for the two nodes at either end of each branch
  branches = read.csv("branches.csv") 
  lifespan = branches$mean
  
  
  ## Usage: Rscript codon.rscript <treefile.ph> <seqfile.fasta> - run codon.rscript.R treefile.ph seqfile.fasta
  ...
  setup = function(beta_0,beta_1,kappa,treefile, seqfile) {
    ...
    ### This needs to change to allow Omega to depend on model Emily needs to modify code here
    Omega = rep(0,nrow(edges))
    for(i in 1:nrow(edges)) {
      Omega[i] = beta_0 + beta_1 * lifespan[i]
    }
  ...
  ## Emily, the next lines run the code - you can modify them to run the code on whatever you want
  ...
  #m0.out = optim(c(2,0.1,0,1),lik_fun,model=0,treefile=treefile,seqfile=seqfile,control=list(fnscale=-1))
  #m1.out = optim(c(2,0.1,0,1),lik_fun,model=1,treefile=treefile,seqfile=seqfile,control=list(fnscale=-1))
  print(m0.out)
  print(m1.out)
